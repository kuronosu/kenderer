...
// Transform is a translate-rotate-scale transform. Rotation holds Euler angles
// in radians; the composed matrix is T * Rz * Ry * Rx * S, i.e. a vertex is
// scaled, then rotated about X, Y, Z, then translated.
...
func (t Transform) Matrix() math3d.Mat4 {
	rot := math3d.RotateZ(t.Rotation.Z).
		Mul(math3d.RotateY(t.Rotation.Y)).
		Mul(math3d.RotateX(t.Rotation.X))
	return math3d.Translate(t.Position).Mul(rot).Mul(math3d.Scale(t.Scale))
}
